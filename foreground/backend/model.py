...
class OmsOrder(Base):
    ...
    member_id = Column(ForeignKey('ums_member.id'), nullable=False, index=True)
...
class PmsProduct(Base):
    ...
    id = Column(BIGINT(20), primary_key=True)
    brand_id = Column(ForeignKey('pms_brand.id'), index=True)
    product_category_id = Column(ForeignKey('pms_product_category.id'), index=True)
    feight_template_id = Column(ForeignKey('pms_feight_template.id'), index=True)
    product_attribute_category_id = Column(ForeignKey('pms_product_attribute_category.id'), index=True)
    ...
    shop_id = Column(ForeignKey('pms_shop.id'), index=True)
...
class OmsCartItem(Base):
    ...
    product_category_id = Column(ForeignKey('pms_product_category.id'), index=True, comment='商品分类')
