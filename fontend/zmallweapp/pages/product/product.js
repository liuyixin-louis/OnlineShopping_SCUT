...
Page({
  ...
  //  console.log(options);
   var that = this;
   wx.request({
     url: 'http://localhost:5000/get_product/'+options.id,
     success(res){
       // console.log(res.data.res.product);
       // console.log(res.data.res.sku);
       WxParse.wxParse('productDetail', 'html', res.data.res.product.detail, that, 0);
       that.setData({
         product:res.data.res.product,
         sku:res.data.res.sku
       })
       console.log(res.data.res.product.detail);
       console.log("获取product,sku成功！");
     }});
  ...
  //  let that = this;
  console.log(that.data.sku.tree);
      ...
      wx.request({
        ...
        data: {
          ...
          'user_id':that.data.userId,
            ...
            "checked": 1,
            "picUrl": that.data.product.picUrl,
            "title": that.data.product.title,
            "spec": that.data.sku.specTextNoCount,
            "count": that.data.sku.count,
            "maxNum": that.data.sku.quota,
